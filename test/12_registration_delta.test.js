...
describe('[ REGISTRATION DELTA TABLE TEST SUITES ]', () => {
  ...
  it('Registration delta table Infleet Record count matches JSON length and records match', async () => {
    if (!infleetBatchId) throw new Error('No infleet batch ID found');

    const dbRecords = await fetchFleetRecordsForregistrationdelta(infleetBatchId);
